...
func checkGroupOfTwoTrack(data []track, relArr [27][27]bool) {

	// Iterate through each column in db
	for groupOne := 0; groupOne < 27; groupOne++ {

		for groupTwo := 0; groupTwo < 27; groupTwo++ {
			...
			if groupOne != groupTwo {
				...
				for _, elem := range data {
					...
					key := getValueByColumnNumTrack(groupOne, elem)
					key += getValueByColumnNumTrack(groupTwo, elem)
...
func main() {
	...
	// I get from two
	relArr := checkGroupsOfOneTrack(data)
	checkGroupOfTwoTrack(data, relArr)
